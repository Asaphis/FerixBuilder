...
export function SiteShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    <header className="topbar"><Link href="/" className="brand"><span><Sparkles size={15} /></span>Ferix<span>Builder</span></Link><nav className="nav-links">{navItems.map(([label, href]) => <Link key={href} href={href} className={location === href ? "active" : ""}>{label}</Link>)}</nav><div className="top-actions"><Link href="/contact" className="contact-link"><MessageCircle size={15} /> Talk to us</Link><Link href="/dashboard" className="login-link"><LogIn size={14} /> Log in</Link><Link href="/register" className="start-link">Create account <UserPlus size={14} /></Link><button className="menu-toggle" onClick={() => setOpen(!open)} aria-label="Open navigation">{open ? <X /> : <Menu />}</button></div></header>
    <AnimatePresence>{open && <motion.nav className="mobile-nav" initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: .2 }}>{navItems.map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)}>{label}</Link>)}<Link href="/contact" onClick={() => setOpen(false)}>Talk to us</Link><Link href="/dashboard" onClick={() => setOpen(false)}>Log in</Link><Link href="/register" onClick={() => setOpen(false)}>Create account</Link></motion.nav>}</AnimatePresence>
    {children}
